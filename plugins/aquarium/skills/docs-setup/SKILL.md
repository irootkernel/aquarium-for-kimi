...
2. Discover the public root README plus documentation, roadmap, specification, architecture, ADR, implementation, operations, TODO dossier, deferred-feedback, archive, contract, validation, and language authorities before asking the user for facts.
...
4. Resolve this skill's directory and run `python3 <skill-directory>/scripts/inspect_docs.py --repository <git-root>`. Treat its `aquarium-docs-inspection/v2` JSON as conservative structural discovery only. It reports role ownership, explicit roadmap units and links, exclusions, and unambiguous structural conflicts; it does not interpret prose, decide migration eligibility, certify documentation completeness, or establish current product truth.
...
- `audit`: report the current profile, public-versus-maintainer audience boundary, role owners, operations coverage, roadmap units, active dossier and completed-outcome references, conflicts, missing roles, and unverifiable facts without drafting or mutation.
...
- `adopt`: preserve existing paths and identifiers, add or reconcile the root documentation index, fill only required role gaps approved by the user, and retrofit a pre-contract active epic's missing dossier by atomically reconciling its declaration, adopted TODO index entry, and roadmap `Detailed SOT` link when that lifecycle gap is explicitly selected.
...
For every mutating operation, produce one decision-complete proposal containing the selected profile, scopes, public-versus-maintainer audience boundary, role-to-path map, operations ownership, precedence, roadmap namespaces, ID policy, dossier lifecycle, exact target paths, moves versus new files, reference rewrites, validation commands, and exclusions.
...
Return the operation, profile, scopes, audience boundary, role owners, operations coverage, canonical roadmaps, ID policy, dossier lifecycle findings, migration mapping if any, changed paths, checks with exit codes, unresolved gaps, and separate staging, commit, and publication state.
