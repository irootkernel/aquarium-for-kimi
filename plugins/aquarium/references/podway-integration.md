...
Read this reference whenever `task-handler`, `epic-handler`, `epic-validator`, `new-project`, `new-feature`, `refactor`, `war-room`, or `design-qa` uses its default Podway path. Skip it only when the current user explicitly opts this workflow out before its managed session starts or a higher-priority instruction prohibits Podway. A non-Git `new-project` also skips Podway completely. Podway records and guards procedure state; it does not run commands, validate the truth of evidence, mutate Git, or replace repository authority.
