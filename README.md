...
The `Codex` name is otherwise forbidden in generated text. `tool-catalog.md` is exempt because it names the Codex CLI as a Mulgae provider and a required CLI version, and `provider-contracts.md` because it names the same CLI as one Orca review provider; both stay true here. Each exemption records the upstream digest it was judged against, so the sync stops when that file changes.
