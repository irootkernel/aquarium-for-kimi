...
| `skills/dev-setup/references/tool-catalog.md` | Registers Mulgae, Gaori, and Ouroboros MCP servers in `mcp.json` (millisecond `startupTimeoutMs`/`toolTimeoutMs` fields) and verifies them by reading the effective configuration plus `kimi doctor`, because Kimi Code has no `mcp get` CLI probe. |
